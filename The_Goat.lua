credits = [[
 ...
 _________  ___  ___  _______           ________  ________  ________  _________   
     ...
     \ \  \ \ \   __  \ \  \_|/__       \ \  \  __\ \  \\\  \ \   __  \   \ \  \  
      \ \  \ \ \  \ \  \ \  \_|\ \       \ \  \|\  \ \  \\\  \ \  \ \  \   \ \  \ 
       \ \__\ \ \__\ \__\ \_______\       \ \_______\ \_______\ \__\ \__\   \ \__\
        ...
        Make your money the easy way! -- Original: FMTransactionTriggerer
]]
...
log.info(credits)
transactionTriggers = gui.add_tab("The Goat")

trigger_transactions = false
current_transaction = 0

transactionTriggers:add_imgui(function()
    ...
    trigger_transactions = ImGui.Checkbox("Trigger Transactions", trigger_transactions)
	toolTip("", "Iterates over every single transaction in the game every second.")
	
	ImGui.SameLine()
	removeError = ImGui.Checkbox("Block Transaction Errors", removeError)
	toolTip("", "Blocks/Removes transaction errors")
end)

script.register_looped("transactions", function(script)
    if not trigger_transactions then
        script:yield()
        return
    end

    if current_transaction < #transactions then
        local hash  = joaat(transactions[current_transaction + 1])
        ...
        --gui.show_message("Service Transactions", "All transactions processed.")
        trigger_transactions = false
        current_transaction = 0
    end
    script:sleep(10) -- Wait 1 seconds before checking again
end)

script.register_looped("removeError", function(script)
	if removeError then
		globals.set_int(4537455, 0)
	end
end)
